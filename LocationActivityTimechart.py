...
# TODO: show a legend saying that colors are shaded dark / white for Sleeping / Work
...
def matchColor(currentLocation):
	...
	elif type(currentLocation) == tuple:
		if currentLocation[0] in ACTIVITIES:
			return ACTIVITIES[currentLocation[0]]
		elif currentLocation[1] in CATEGORIES:
			return CATEGORIES[currentLocation[1]]
...
def main(input_location_csv, input_schedule_csv, output_chart_name):
	...
	for line in open(input_schedule_csv, 'rU'):
		...
		startTime = datetime.datetime.strptime(startDate + ' ' + startTime, '%Y-%m-%d %H:%M:%S')
		durationSplit = duration.split(':')
		duration = int(durationSplit[0]) * 60 * 60 + int(durationSplit[1]) * 60
		if activity in ACTIVITIES or category in CATEGORIES:
			data.append(((activity, category), duration, startTime))
...
if __name__ == "__main__":
	if len(sys.argv) < 4:
		print 'Usage: ' + sys.argv[0] + ' input_location_csv input_schedule_csv output_chart_name'
	else:
